    ...
    @component(layoutForm() , ['title' => trans('categories.categories') , 'model' => 'categories' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
        ...
        <form class="card-body" action="{{ concatenateLangToUrl('lazyadmin/categories/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
		 ...
		 <div class="form-group {{ $errors->has("show_home") ? "has-error" : "" }}" > 
			...
			<div class="form-group {{ $errors->has("futurework") ? "has-error" : "" }}" >
				<label class="col-md-2 col-form-label" for="futurework">futurework</label>
				<div class="form-check">
					<label class="form-check-label">
						<input class="form-check-input" name="futurework" {{ isset($item->futurework) && $item->futurework == 0 ? "checked" : "" }} type="radio" value="0" >
						{{ trans("categories.No")}}
					</label >
					<label class="form-check-label">
						<input class="form-check-input" name="futurework" {{ isset($item->futurework) && $item->futurework == 1 ? "checked" : "" }} type="radio" value="1" >
						{{ trans("categories.Yes")}}
					</label>
				</div> 		</div>
			@if ($errors->has("futurework"))
				<div class="alert alert-danger">
					<span class='help-block'>
						<strong>{{ $errors->first("futurework") }}</strong>
					</span>
				</div>
			@endif


			<div class="form-group {{ $errors->has("futurecode") ? "has-error" : "" }}" >
				<label class="col-md-2 col-form-label" for="futurecode">{{ trans("categories.futurecode")}}</label>
				<input type="text" name="futurecode" class="form-control" id="futurecode" value="{{ isset($item->futurecode) ? $item->futurecode : old("futurecode") }}"  placeholder="{{ trans("categories.futurecode")}}">
			</div>
			@if ($errors->has("futurecode"))
				<div class="alert alert-danger">
					<span class='help-block'>
						<strong>{{ $errors->first("futurecode") }}</strong>
					</span>
				</div>
			@endif
