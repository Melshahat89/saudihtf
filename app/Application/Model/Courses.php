...
//        static::addGlobalScope(new SubscriptionScope());
    ...
    protected $fillable = [
        ...
        'vdocipher_tag', 'webinar_url', 'tags','file','paragraph','alt_image','prefix_vimeo','canonical','futurexid'
        ,'subscriptionplatform','featured_on_subscription'
    ];
...
//        }
    ...
    public function getTotalResourcesCount(){
        ...
        foreach($this->courseincludes as $includedCourse){
            $sum += optional(optional($includedCourse->includedCourse)->courseresources)->count() ?? 0;
        }
        return $sum;
    }
    public function getHoursLectures($minutes = NULL)
    {
        ...
        foreach($this->courseincludes as $includedCourse){
            $sum += optional(optional($includedCourse->includedCourse)->courselectures)->sum('length') ?? 0;
        }
        if($sum >= 86400){
